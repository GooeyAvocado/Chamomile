            ...
            album.thumbId
                ...
                : <div style={{ display: 'flex', flexWrap: "wrap", width: "100%", height: "100%", aspectRatio: "2/1", objectFit: 'cover', objectPosition: 'center top', position: 'absolute', left: '0', top: '0' }}>
                    {album.firstFourImages?.map((a) =>
                        <img key={`albumsample-${a}`} src={imageUrl(a)} style={{ flex: '1', width: "100%", height: (album.firstFourImages?.length ?? 0) < 3 ? "100%" : "50%", aspectRatio: "2/1", objectFit: "cover", objectPosition: 'center top' }} />
                    )}
                </div>
        }
    </div>
